...
class CarAgentoOptimizer(Agent):
            ...
            if self.model.grid.out_of_bounds((self.pos[0], self.pos[1] + self.speed)):
                ...
                if self.choosen == True and self.steps >= STOP:
                    ...
                    if self.check_forward() == 0:
                        ...
                        if self.model.choosen_agent == True and self.pos[0] == 1 and self.check_above() and self.choosen == False and self.steps > 6:
                            self.model.grid.move_agent(
                                self, (self.pos[0] + 1, self.pos[1] + 1))
                            self.i_changed = True
                        if self.model.choosen_agent == True and self.pos[0] == 1 and self.check_below() and self.choosen == False and self.steps > 6:
                            self.model.grid.move_agent(
                                self, (self.pos[0] - 1, self.pos[1] + 1))
                            self.i_changed = True
                        if self.i_changed == True and (self.steps > (STOP + 2)):
                            if self.pos[0] == 0:
                                self.model.grid.move_agent(
                                    self, (self.pos[0] + 1, self.pos[1] + 1))
                            elif self.pos[0] == 2:
